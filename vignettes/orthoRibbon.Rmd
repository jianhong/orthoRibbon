...
## Start from the ortholog groups information

Another OrthoRibbon supported workflow starts from ortholog group information, where each group contains genes from different species that are inferred to be orthologous. The following examples demonstrate how to generate ribbon diagrams using ortholog groups by retrieving the corresponding gene annotations and genomic coordinates for visualization.

First we prepare the species abbreviation.
```{r human_fly_cn}
com_name <- c('hsapiens', 'dmelanogaster')
names(com_name) <- com_name # must assign the names.
```

And then load the ortholog data. You can get the ortholog data via tools like
...
Finally, prepare the plotting data and generate the ribbon diagram.
